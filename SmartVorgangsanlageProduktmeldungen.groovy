...
def String linkGussberichtVorlage = Constants.BASE_URL_CONFLUENCE + "/display/Giess/Gussbericht?run_1_Gussnummer=@@Gussnummer@@&run_1_Anlage=@@Anlage@@&run_1=run"
def String linkGussbericht=""
def String linkChargenberichtVorlage = Constants.BASE_URL_CONFLUENCE + "/display/PRODWW/Chargenbericht?run_1_Chargennummer=@@Chargennummer@@&run_1=run"
...
if (rsArbeitsvorrat.isEmpty()==false) {
    rsArbeitsvorrat.each { block ->
        ...
        if (anzahlBlockmeldungenZurCharge==0) {
            ...
            // Link Gussbericht: cf_10802
            linkGussbericht = linkGussberichtVorlage.replace("@@Gussnummer@@",dfGanzahlig.format(block.gussnr)).replace("@@Anlage@@",dfGanzahlig.format(block.giessanlagennr))
            MapFields.put ("customfield_10802",linkGussbericht)
            // Link Chargenbericht: cf_10805
            linkChargenbericht = linkChargenberichtVorlage.replace("@@Chargennummer@@",block.chargennr.replace("/","%2F"))
            MapFields.put ("customfield_10805",linkChargenbericht)
