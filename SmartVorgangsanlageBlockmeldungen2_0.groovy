...
* Wir suchen als erstes Güsse mit Legierung 999 und legen für diese grundsätzlich Meldungen an; wenn der Fertigungsauftrag der "Gussabbruch"-Auftrag ist, legen wir zusätzlich einen Fehlerkatalog-Eintrag "Gussabbruch" an
* 
 */
...
def abfrageArbeitsvorratSpuelguesse = """
  select
    substr(mat.CHARGENNR,1,7) gussnr
  ...
  where smcb.DISCRIMINATOR='SMCASTINGBATCH' and
    ...
    mat.ADDTIME>to_date('2016-05-01','YYYY-MM-DD') and
...
def abfrageArbeitsvorratBewertung = """
  select
    substr(mat.chargennr,1,7) gussnr
  from fs4p.material mat
  join fs4p.schedulable genop on genop.OBJID=mat.GENERATINGOPERATIONOID
...
//zuerst Spülgüsse suchen
def rsArbeitsvorrat = sql.rows(abfrageArbeitsvorratSpuelguesse)

// wenn es keine Spülgüsse anzulegen gibt: Chargen mit schlechter Bewertung suchen, die keine Spülgüsse sind
if (rsArbeitsvorrat.isEmpty()==true) {
  rsArbeitsvorrat = sql.rows(abfrageArbeitsvorratBewertung)
}
